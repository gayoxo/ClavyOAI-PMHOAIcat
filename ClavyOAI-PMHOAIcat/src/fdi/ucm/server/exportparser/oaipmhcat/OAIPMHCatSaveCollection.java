...
public class OAIPMHCatSaveCollection extends SaveCollection {
	...
	private static final String ODA = "OAI-PMH en OAICat (MySQL only)";
	...
	public CompleteLogAndUpdates processCollecccion(CompleteCollection Salvar,
		...
		try {
			...
			SaveProcessMainOAIPMHCat oda = new SaveProcessMainOAIPMHCat(Salvar,CL,PathFile,Controlado);
			oda.preocess();
	...
	public ArrayList<ImportExportPair> getConfiguracion() {
		if (Parametros==null)
		{
			...
			ListaCampos.add(new ImportExportPair(ImportExportDataEnum.Boolean, "Controlado Only Synonyms?"));
			ListaCampos.add(new ImportExportPair(ImportExportDataEnum.File, "File of Synonyms",true));
	...
	public void setConfiguracion(ArrayList<String> DateEntrada) {
		if (DateEntrada!=null)
		{
			...
			Controlado=Boolean.parseBoolean(DateEntrada.get(6));
			
			
			if (DateEntrada.size()>6)
				PathFile=DateEntrada.get(7);
			else 
				PathFile="";
		}
		}
